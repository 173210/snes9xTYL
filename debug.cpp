...
void ProcessDebugCommand (char *Line)
{
	...
	for (int i = 0; i < 0x40; i += 2)
	{
	    if (i % 16 == 0)
		printf ("\n%04x ", 0xffc0 + i);
	    printf ("%04x ", (APUPack.APU.ExtraRAM) [i]);
	}
	printf ("\n");
    }
    if (Line[0] == 's')
	...
	for (int p = 0; p < 4; p++)
	{
	    int c = 0;
	    int i;
	    for (i = 0; GPUPack.GFX.OBJList [i] >= 0; i++)
	    {
		if (PPU.OBJ[GPUPack.GFX.OBJList [i]].Priority == p)
		    c++;
	    }
	...
	if ((APUPack.APU.Flags) & TRACE_FLAG)
	{
#ifdef SPCTOOL
	    printf ("ENABLED\n");
	...
	printf ("APU tracing %s\n", (APUPack.APU.Flags) & TRACE_FLAG ? "enabled" :
		"disabled");
    }
    if (*Line == 'B')
    {
...
#else
	printf ("APU out-ports: %02X %02X %02X %02X\n",
		(APUPack.APU.OutPorts) [0], (APUPack.APU.OutPorts) [1], (APUPack.APU.OutPorts) [2], (APUPack.APU.OutPorts) [3]);
#endif
	printf ("ROM/RAM switch: %s\n", ((IAPUuncached.RAM) [0xf1] & 0x80) ? "ROM" : "RAM");
	for (int i = 0; i < 3; i++)
	    if ((APUPack.APU.TimerEnabled) [i])
		printf ("Timer%d enabled, Value: 0x%03X, 4-bit: 0x%02X, Target: 0x%03X\n",
			i, (APUPack.APU.Timer) [i], (IAPUuncached.RAM) [0xfd + i], (APUPack.APU.TimerTarget) [i]);
    }
    if (*Line == 'P')
    {
