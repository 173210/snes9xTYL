...
unsigned char cle[32]; /* global variables */
...
if ((in=fopen("in.bin","rb")) == NULL) {printf("\nError reading file IN.BIN !\n");fin();}
if ((out=fopen("out.bin","wb")) == NULL) {printf("\nError writing file OUT.BIN !\n");fin();}

/* the crypted file is in OUT.BIN */
...
c = c ^ (cfc^cfd);
fputc(c,out); /* we write the crypted byte in the file OUT.BIN */
}
fclose (in);
fclose (out);
fin();
