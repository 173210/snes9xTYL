...
GPUPack.GFX.X2 [((((C1) & yoRGB_REMOVE_LOW_BITS_MASK) + \
...
GPUPack.GFX.ZERO [(((C1) | yoRGB_HI_BITS_MASKx2) - \
	   ((C2) & yoRGB_REMOVE_LOW_BITS_MASK)) >> 1]
...
{
   	    uint16 *Screen = (uint16 *) GPUPack.GFX.Screen + Offset - 7;
   	    uint16 *SubScreen = (uint16 *) GPUPack.GFX.SubScreen + Offset - 7;
   	    uint8 *ZB = (uint8 *)GPUPack.GFX.ZBuffer + Offset - 7;
...
void DrawTile16New (uint32 Tile, uint32 Offset, uint32 StartLine, uint32 LineCount)
{
    ...
    if (BG.DirectColourMode)
    {
        ...
        GPUPack.GFX.ScreenColors = DirectColourMaps [(Tile >> 10) & BG.PaletteMask];
        Col = 0;
    }
    else 
    {
    ...
    if ((!BG.Buffered [TileNumber<<1])|(BG.Buffered [(TileNumber<<1)|1]!=Col)) 
   	{
   		BG.Buffered[TileNumber<<1] = ConvertTile16New (pCache, TileAddr,GPUPack.GFX.ScreenColors);
	...
	if ( (BG.Buffered [TileNumber<<1] == 2)&&(!solid_lineclip))
	{
	    ...
	    if (!(Tile & (V_FLIP | H_FLIP)))
	    {
		    //NO FLIP
#ifdef asmPPU16
			asmDrawTile16((uint32)pCache,0,(uint32)(GPUPack.GFX.S+(Offset<<1)),0);			
#else
			bp = pCache+8;
	    ...
	    if (!(Tile & V_FLIP))
	    {
	    	//HFLIP
#ifdef asmPPU16
			asmDrawTile16((uint32)pCache,0,(uint32)(GPUPack.GFX.S+(Offset<<1)+7*2),2);			
#else			
			bp = pCache+8;			
			Offset += 7;			
	    	for (l = 8; l != 0; l--, bp += 8*2, Offset--)
	    ...
	    if (Tile & H_FLIP)
	    {
	    	//VFLIP & HFLIP
#ifdef asmPPU16
			asmDrawTile16((uint32)pCache,0,(uint32)(GPUPack.GFX.S+(Offset<<1)+7*2),3);	   		
#else	   		
			bp = pCache+8;
			Offset += 7;
	    ...
	    else
	    {
		    //VFLIP
#ifdef asmPPU16
			asmDrawTile16((uint32)pCache,0,(uint32)(GPUPack.GFX.S+(Offset<<1)),1);
#else	   		
			bp = pCache+8;
	    	for (l = 8; l != 0; l--, bp += 8*2, Offset++)
		    	FLIPPED16_O (Offset, (uint16*)bp);
	...
	else
	{
	    ...
	    if (!(Tile & (V_FLIP | H_FLIP)))
	    {
		    //NO FLIP
#ifdef asmPPU16
			asmDrawTile16((uint32)pCache,solid_lineclip,(uint32)(GPUPack.GFX.S+(Offset<<1)),0);
#else			
		    headerbp = pCache;
			bp = pCache+8;					
	    ...
	    if (!(Tile & V_FLIP))
	    {
	    	//HFLIP
#ifdef asmPPU16
			asmDrawTile16((uint32)pCache,solid_lineclip,(uint32)(GPUPack.GFX.S+(Offset<<1)+7*2),2);			
#else			
		    headerbp = pCache;
			bp = pCache+8;
			Offset += 7;
...
#ifdef asmPPU16
			asmDrawTile16((uint32)pCache,solid_lineclipI,(uint32)(GPUPack.GFX.S+(Offset<<1)+7*2),3);
#else			
		    headerbp = pCache;
			bp = pCache+8;
			Offset += 7;
...
#ifdef asmPPU16
			asmDrawTile16((uint32)pCache,solid_lineclipI,(uint32)(GPUPack.GFX.S+(Offset<<1)),1);
#else			
		    headerbp = pCache;
			bp = pCache+8;
	    	for (l = 8; l != 0; l--, bp += 8*2, headerbp++, Offset++)
			...
			uint32 StartLine, uint32 LineCount)
{
	...
	if ( (BG.Buffered [TileNumber<<1] == 2)&&(!solid_lineclip))
	{
	    ...
	    if (!(Tile & (V_FLIP | H_FLIP)))
	    {
		    //NO FLIP
#ifdef asmPPU16
			asmDrawTileClipped16((uint32)pCache,(StartPixel<<16)|(Width<<8),(uint32)(GPUPack.GFX.S+(Offset<<1)+StartPixel*2),0);			
#else			
	    ...
	    if (!(Tile & V_FLIP))
	    {
	    	//HFLIP
#ifdef asmPPU16
			asmDrawTileClipped16((uint32)pCache,(StartPixel<<16)|(Width<<8),(uint32)(GPUPack.GFX.S+(Offset<<1)+(StartPixel+Width-1)*2),2);			
#else					  
			bp = pCache+8+StartPixel*16;
	    ...
	    if (Tile & H_FLIP)
	    {
	    	//VFLIP & HFLIP
#ifdef asmPPU16
			asmDrawTileClipped16((uint32)pCache,(StartPixel<<16)|(Width<<8),(uint32)(GPUPack.GFX.S+(Offset<<1)+(StartPixel+Width-1)*2),3);
#else			
	    ...
	    else
	    {
		    //VFLIP
#ifdef asmPPU16
			asmDrawTileClipped16((uint32)pCache,(StartPixel<<16)|(Width<<8),(uint32)(GPUPack.GFX.S+(Offset<<1)+StartPixel*2),1);
#else			
	...
	else
	{
	    ...
	    if (!(Tile & (V_FLIP | H_FLIP)))
	    {
		    //NO FLIP
#ifdef asmPPU16
			asmDrawTileClipped16((uint32)pCache,(StartPixel<<16)|(Width<<8)|solid_lineclip,(uint32)(GPUPack.GFX.S+(Offset<<1)+StartPixel*2),0);			
#else			
	    ...
	    if (!(Tile & V_FLIP))
	    {
	    	//HFLIP
#ifdef asmPPU16
			asmDrawTileClipped16((uint32)pCache,(StartPixel<<16)|(Width<<8)|solid_lineclip,(uint32)(GPUPack.GFX.S+(Offset<<1)+(StartPixel+Width-1)*2),2);			
#else			
...
#ifdef asmPPU16
			asmDrawTileClipped16((uint32)pCache,(StartPixel<<16)|(Width<<8)|solid_lineclipI,(uint32)(GPUPack.GFX.S+(Offset<<1)+(StartPixel+Width-1)*2),3);			
#else
...
#ifdef asmPPU16
			asmDrawTileClipped16((uint32)pCache,(StartPixel<<16)|(Width<<8)|solid_lineclipI,(uint32)(GPUPack.GFX.S+(Offset<<1)+StartPixel*2),1);			
#else		    
		    headerbp = pCache+StartPixel;
			bp = pCache+8+StartPixel*16;
			Offset += StartPixel;
