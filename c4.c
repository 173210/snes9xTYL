...
void C4TransfWireFrame ()
{
    ...
    c4y2 = c4y * vfpu_cosf (tanval) - c4z * vfpu_sinf (tanval);
    c4z2 = c4y * vfpu_sinf (tanval) + c4z * vfpu_cosf (tanval);
    
    // Rotate Y
    tanval = -(float)C4WFY2Val*3.14159265*2/128;
    c4x2 = c4x * vfpu_cosf (tanval) + c4z2 * vfpu_sinf (tanval);
    c4z = c4x * - vfpu_sinf (tanval) + c4z2 * vfpu_cosf (tanval);
    
    // Rotate Z
    tanval = -(float) C4WFDist * 3.14159265*2 / 128;
    c4x = c4x2 * vfpu_cosf (tanval) - c4y2 * vfpu_sinf (tanval);
    c4y = c4x2 * vfpu_sinf (tanval) + c4y2 * vfpu_cosf (tanval);
...
void C4TransfWireFrame2 ()
{
	...
	*/
	...
	// Rotate X
    tanval = -(float) C4WFX2Val * 6.2831853 / 128;
    c4y2 = c4y * vfpu_cosf (tanval) - c4z * vfpu_sinf (tanval);
    c4z2 = c4y * vfpu_sinf (tanval) + c4z * vfpu_cosf (tanval);
    
    // Rotate Y
    tanval = -(float)C4WFY2Val * 6.2831853 / 128;
    c4x2 = c4x * vfpu_cosf (tanval) + c4z2 * vfpu_sinf (tanval);
    c4z = c4x * - vfpu_sinf (tanval) + c4z2 * vfpu_cosf (tanval);
    
    // Rotate Z
    tanval = -(float) C4WFDist * 6.2831853 / 128;
    c4x = c4x2 * vfpu_cosf (tanval) - c4y2 * vfpu_sinf (tanval);
    c4y = c4x2 * vfpu_sinf (tanval) + c4y2 * vfpu_cosf (tanval);
...
void C4CalcWireFrame ()
{
    ...
    if (vfpu_abs ((float)C4WFXVal) > vfpu_abs ((float)C4WFYVal))
    {
        C4WFDist = vfpu_abs ((float)C4WFXVal) + 1;
        C4WFYVal = (short) (256 * (float) C4WFYVal / vfpu_abs ((float)C4WFXVal));
    ...
    else
    {
        if (C4WFYVal != 0) 
        {
            C4WFDist = vfpu_abs((float)C4WFYVal)+1;
            C4WFXVal = (short) (256 * (float)C4WFXVal / vfpu_abs ((float)C4WFYVal));
            if (C4WFYVal < 0)
                C4WFYVal = -256;
            else 
                C4WFYVal = 256;
...
void C4Op1F ()
{
    ...
    else 
    {
        ...
        //C41FAngleRes = (short) (vfpu_atanf (tanval) / (3.141592675 * 2) * 512);
		C41FAngleRes = (short) (vfpu_atanf (tanval) / (6.2831853) * 512);
