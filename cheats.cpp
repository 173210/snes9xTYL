                         ...
                         S9xCheatDataSize size, bool8 is_signed, bool8 update)
{
    ...
    if (is_signed)
    {
        for (i = 0; i < 0x20000 - l; i++)
        {
            if (TEST_BIT (d->WRAM_BITS, i) &&
                _s9xC(cmp, _DS(size, d->RAM, i), _DS(size, d->CWRAM, i)))
            {
                if (update)
                    d->CWRAM [i] = d->RAM [i];
            }
        ...
        for (i = 0; i < 0x10000 - l; i++)
        {
            if (TEST_BIT (d->SRAM_BITS, i) &&
                _s9xC(cmp, _DS(size, d->SRAM, i), _DS(size, d->CSRAM, i)))
            {
                if (update)
                    d->CSRAM [i] = d->SRAM [i];
            }
        ...
        for (i = 0; i < 0x2000 - l; i++)
        {
            if (TEST_BIT (d->IRAM_BITS, i) &&
                _s9xC(cmp, _DS(size, d->FillRAM + 0x3000, i), _DS(size, d->CIRAM, i)))
            {
                if (update)
    ...
    else
    {
        ...
        {
            if (TEST_BIT (d->WRAM_BITS, i) &&
                _s9xC(cmp, _D(size, d->RAM, i), _D(size, d->CWRAM, i)))
            {
                if (update)
                    d->CWRAM [i] = d->RAM [i];
            }
        ...
        {
            if (TEST_BIT (d->SRAM_BITS, i) &&
                _s9xC(cmp, _D(size, d->SRAM, i), _D(size, d->CSRAM, i)))
            {
                if (update)
                    d->CSRAM [i] = d->SRAM [i];
            }
        ...
        {
            if (TEST_BIT (d->IRAM_BITS, i) &&
                _s9xC(cmp, _D(size, d->FillRAM + 0x3000, i), _D(size, d->CIRAM, i)))
            {
                if (update)
                    d->CIRAM [i] = d->FillRAM [i + 0x3000];
            }
                        ...
                        bool8 is_signed, bool8 update)
{
    ...
    if (is_signed)
    {
        ...
        {
            if (TEST_BIT (d->WRAM_BITS, i) &&
                _s9xC(cmp, _DS(size, d->RAM, i), (int32) value))
            {
                if (update)
                    d->CWRAM [i] = d->RAM [i];
            }
        ...
        {
            if (TEST_BIT (d->SRAM_BITS, i) &&
                _s9xC(cmp, _DS(size, d->SRAM, i), (int32) value))
            {
                if (update)
                    d->CSRAM [i] = d->SRAM [i];
            }
        ...
        {
            if (TEST_BIT (d->IRAM_BITS, i) &&
                _s9xC(cmp, _DS(size, d->FillRAM + 0x3000, i), (int32) value))
            {
                if (update)
                    d->CIRAM [i] = d->FillRAM [i + 0x3000];
            }
    ...
    else
    {
        ...
        {
            if (TEST_BIT (d->WRAM_BITS, i) &&
                _s9xC(cmp, _D(size, d->RAM, i), value))
            {
                if (update)
                    d->CWRAM [i] = d->RAM [i];
            }
        ...
        {
            if (TEST_BIT (d->SRAM_BITS, i) &&
                _s9xC(cmp, _D(size, d->SRAM, i), value))
            {
                if (update)
                    d->CSRAM [i] = d->SRAM [i];
            }
        ...
        {
            if (TEST_BIT (d->IRAM_BITS, i) &&
                _s9xC(cmp, _D(size, d->FillRAM + 0x3000, i), value))
            {
                if (update)
                    d->CIRAM [i] = d->FillRAM [i + 0x3000];
            }
