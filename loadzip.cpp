	      ...
	      int32 *headers)
{
    ...
    *headers = 0;
    ...
    char *ext,*_ext;
    ext = strchr(filename,'.');
    while (_ext=strchr(ext+1,'.')) ext=_ext;
    ...
    {
	if (info.uncompressed_size > CMemory::MAX_ROM_SIZE + 512){
		S9xMessage(0,0,"unzipped file too big");	
	}
	...
	int l = unzReadCurrentFile(file,ptr,FileSize,psp_showProgressBar );
	...
	int len;
	if (ptr - ROM < CMemory::MAX_ROM_SIZE + 0x200 &&
	    (isdigit (ext [0]) && ext [1] == 0 && ext [0] < '9'))
	{
	    more = TRUE;
	    ext [0]++;
	}
	else if (ptr - ROM < CMemory::MAX_ROM_SIZE + 0x200 &&
