...
void S9xPrintAPUState ()
{
    ...
    printf ("Noise: %s, Frequency: %d, Pitch mod: %s\n", as_binary ((APUPack.APU.DSP) [APU_NON]),
	    NoiseFreq [(APUPack.APU.DSP) [APU_FLG] & 0x1f],
	...
	{
	    int freq = ch->hertz;
	    if ((APUPack.APU.DSP) [APU_NON] & (1 << J)) //ch->type == SOUND_NOISE)
	    {
		freq = NoiseFreq [(APUPack.APU.DSP) [APU_FLG] & 0x1f];
		printf ("noise, ");
	    }
	    else
		printf ("sample %d, ", (APUPack.APU.DSP) [APU_SRCN + J * 0x10]);
