...
#define Relative()\
    ...
    short s9xInt16 = (int) (((IAPU.PC)) + 2 - ((IAPU.RAM))) + s9xInt8;

#define Relative2()\
    signed char s9xInt8 = OP2;\
    short s9xInt16 = (int) (((IAPU.PC)) + 3 - ((IAPU.RAM))) + s9xInt8;
...
{ \
    ((IAPU.PC)) = ((IAPU.RAM)) + (uint16) s9xInt16; \
    ((APUPack.APU.Cycles)) += ((IAPU.TwoCycles)); \
} \
else \
    ((IAPU.PC)) += 3
...
// XXX:Where is the BRK vector ???
    ((IAPU.PC)) = ((IAPU.RAM)) + ((APUPack.APU.ExtraRAM))[0x20] + (((APUPack.APU.ExtraRAM))[0x21] << 8);
#endif
}
...
// CMPW YA,dp
    uint16 Work16 = S9xAPUGetByteZ (OP1) + (S9xAPUGetByteZ (OP1 + 1) << 8);
    long s9xInt32 = (long) (APURegisters.YA).W - (long) Work16;
    ((IAPU._Carry)) = s9xInt32 >= 0;
...
// SUBW YA,dp
    ...
    if ((((APURegisters.YA).W ^ Work16) & 0x8000) &&
	    (((APURegisters.YA).W ^ (uint16) s9xInt32) & 0x8000))
	APUSetOverflow ();
    else
	APUClearOverflow ();
    if ((((APURegisters.YA).W ^ Work16) & 0x0080) &&
	    (((APURegisters.YA).W ^ (uint16) s9xInt32) & 0x0080))
	APUSetHalfCarry ();
    APUSetHalfCarry ();
    if(((APURegisters.YA).W ^ Work16 ^ (uint16) s9xInt32) & 0x10)
        APUClearHalfCarry ();
    (APURegisters.YA).W = (uint16) s9xInt32;
    APUSetZN16 ((APURegisters.YA).W);
    ((IAPU.PC)) += 2;
}
