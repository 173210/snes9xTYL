...
void S9xFixSA1AfterSnapshotLoad ()
{
    ...
    S9xSA1UnpackStatus ();
    S9xSA1FixCycles ();
    SA1Pack.SA1.VirtualBitmapFormat = (FillRAM [0x223f] & 0x80) ? 2 : 4;
...
uint8 S9xSA1GetByte (uint32 address)
{
	...
	address -= 0x600000;
	if (SA1Pack.SA1.VirtualBitmapFormat == 2)
...
/*    uint8 *GetAddress = SA1Pack.SA1.Map [(address >> MEMMAP_SHIFT) & MEMMAP_MASK];
    ...
    case CMemory::MAP_BWRAM:
	return (*(SA1Pack.SA1.BWRAM + ((address+1 & 0x7fff) - 0x6000))) << 8 | (*(SA1Pack.SA1.BWRAM + ((address & 0x7fff) - 0x6000)));
    case CMemory::MAP_BWRAM_BITMAP:
	address -= 0x600000;
	if (SA1Pack.SA1.VirtualBitmapFormat == 2)
	...
	address = (address & 0xffff) - 0x6000;
	if (SA1Pack.SA1.VirtualBitmapFormat == 2)
	    return ((SA1Pack.SA1.BWRAM [(address+1 >> 2) & 0xffff] >> ((address+1 & 3) << 1)) & 3)<<8 | ((SA1Pack.SA1.BWRAM [(address >> 2) & 0xffff] >> ((address & 3) << 1)) & 3);
	else
	    return ((SA1Pack.SA1.BWRAM [(address+1 >> 1) & 0xffff] >> ((address+1 & 1) << 2)) & 15)<<8 | ((SA1Pack.SA1.BWRAM [(address >> 1) & 0xffff] >> ((address & 1) << 2)) & 15);
...
/*	uint8 *Setaddress = SA1Pack.SA1.WriteMap [(address >> MEMMAP_SHIFT) & MEMMAP_MASK];
	...
	*(SRAM + (address+1 & 0xffff)) = Word>>8;
	return;
    case CMemory::MAP_BWRAM:
	*(SA1Pack.SA1.BWRAM + ((address & 0x7fff) - 0x6000)) = Word;
	*(SA1Pack.SA1.BWRAM + ((address+1 & 0x7fff) - 0x6000)) = Word>>8;
	return;
    case CMemory::MAP_BWRAM_BITMAP:
	address -= 0x600000;
	if (SA1Pack.SA1.VirtualBitmapFormat == 2)
	...
	if (SA1Pack.SA1.VirtualBitmapFormat == 2)
	{
	    ...
	    *ptr &= ~(3 << ((address & 3) << 1));
	    *ptr |= (Word & 3) << ((address & 3) << 1);
	    ptr = &SA1Pack.SA1.BWRAM [(address+1 >> 2) & 0xffff];
	    *ptr &= ~(3 << ((address+1 & 3) << 1));
	    *ptr |= (Word>>8 & 3) << ((address+1 & 3) << 1);
	}
	else
	{
	    ...
	    *ptr &= ~(15 << ((address & 1) << 2));
	    *ptr |= (Word & 15) << ((address & 1) << 2);
	    ptr = &SA1Pack.SA1.BWRAM [(address+1 >> 1) & 0xffff];
	    *ptr &= ~(15 << ((address+1 & 1) << 2));
	    *ptr |= (Word>>8 & 15) << ((address+1 & 1) << 2);
	}
    default:
...
void S9xSetSA1MemMap (uint32 which1, uint8 map)
{
	...
	for (i = c; i < c + 16; i++)
	    Memory.Map [start + i] = SA1Pack.SA1.Map [start + i] = block;
    }
	...
	for (i = c + 8; i < c + 16; i++)
	    Memory.Map [start2 + i] = SA1Pack.SA1.Map [start2 + i] = block;
    }
}
...
//	printf ("I-RAM SA1 write protect area %02x\n", byte);
	...
	break;
    case 0x223f:
	SA1Pack.SA1.VirtualBitmapFormat = (byte & 0x80) ? 2 : 4;
