...
menu_time_t os9xpsp_debugmenu[DEBUGMENU_ITEMS]={	
	{"Rendering passes : ",10,NULL,&os9x_showpass,2,{0,1},0,{"Off","On"}},
	{"--------",-1,NULL,NULL,0,NULL,0,NULL},
	{"Speed hacks : ",11,NULL,&os9x_applyhacks,2,{0,1},0,{"Off","On"}},
	{"--------",-1,NULL,NULL,0,NULL,0,NULL},
	{"Fixed Colour : ",12,NULL,&os9x_ignore_fixcol,2,{0,1},0,{"Off","On"}},
	{"Windows clipping : ",13,NULL,&os9x_ignore_winclip,2,{0,1},0,{"Off","On"}},
	{"Add/Sub modes : ",14,NULL,&os9x_ignore_addsub,2,{0,1},0,{"Off","On"}},
	{"Palette writes : ",15,NULL,&os9x_ignore_palwrite,2,{0,1},0,{"Off","On"}},
	{"Hidden details : ",-1,NULL,&os9x_fix_hires,2,{0,1},0,{"Off","On"}},
	{"--------",-1,NULL,NULL,0,NULL,0,NULL},	
...
int show_debugmenu(char *mode) {
    ...
    mh_print(8,0,">" EMUNAME_VERSION "< - Menu",INFOBAR_COL2);
...
int show_inputsmenu(char *mode) {
    ...
    mh_print(8,0,">" EMUNAME_VERSION "< - Inputs Config",INFOBAR_COL2);
    {
    	char status_bar[100];
		...
		if (scePowerIsBatteryExist()) {
		  ...
		  /*
	   	sprintf(bat_info,"%02d%c%02d Bat.:%s%s%s%02d%%%s Tmp.%dC",    		
		  		...
		  		*/
		  		sprintf(bat_info,"%02d%c%02d Bat.:%s%s%s%02d%%%s",    		
		   		tsys->tm_hour,(tsys->tm_sec&1?':':' '),tsys->tm_min,
	   			(scePowerIsPowerOnline()?"Plg.":""),
	   			(scePowerIsBatteryCharging()?"Chrg.":""),
		   		(scePowerIsLowBattery()?"Low!":""),
...
int menu_engine(char *mode) {	
	...
	if (mode) {		
		switch (os9x_softrendering) {
			case 0:sprintf(mode,"Approx.");break;
			case 1:sprintf(mode,"Accur.");break;
			case 2:sprintf(mode,"Fast");break;
			case 3:sprintf(mode,"Fast + Accur.");break;
			case 4:sprintf(mode,"Fast + Approx.");break;			
		}		
		return 0;
	}
	...
	for (;;) {		
		...
		switch (new_value) {
			case 0:sprintf(str_tmp,"Approx.");break;
			case 1:sprintf(str_tmp,"Accur.");break;
			case 2:sprintf(str_tmp,"Fast");break;
			case 3:sprintf(str_tmp,"Fast + Accur.");break;
			case 4:sprintf(str_tmp,"Fast + Approx.");break;			
		}
...
u16 os9x_savestate_mini_[4][128*120];
int menu_loadstate(char *mode) {
	FILE *f;
	char slot_occupied_[4];		
	char ext[5],slot_fileinfo_[4][32];	
	...
	for (i=0;i<4;i++) { //Max save state slots. 4 Currently
		if (i==4) strcpy(ext,".zat");
  	else {
  		strcpy(ext,".za0");
  		ext[3]=i+48;
  	}				
	...
	for (;;) {		
		...
		mh_print_light(250,165,psp_msg_string(MENU_STATE_CHOOSELOAD),31|(31<<5)|(31<<10),menu_current_smoothing);
		...
		for (i=0;i<4;i++) {					  	
			py=29+(i/2)*118;
			if (i==2) {
				if (state_slot>=2) px=15;
				else px=15;
			}
				//Fix the huge Y offset and add a X offset to the icon positioning
		  if (i==state_slot) {
		  	if (slot_occupied_[i]) {
				...
				pgDrawFrame(px+6,py+4,px+6+96+4,py+6+os9x_snesheight*3/8+4,col2);
		  		pgDrawFrame(px+7,py+5,px+7+96+2,py+7+os9x_snesheight*3/8+2,col1);
		  	...
		  	if (slot_occupied_[i]) {
				...
				col1=8|(8<<5)|(8<<10);
				col2=15|(15<<5)|(15<<10);				
				pgDrawFrame(px+6,py+4,px+6+64+4,py+6+os9x_snesheight/4+4,col2);
		  		pgDrawFrame(px+7,py+5,px+7+64+2,py+7+os9x_snesheight/4+2,col1);
    ...
    if (new_pad&PSP_CTRL_LEFT) {
    	if (state_slot%2) state_slot--;
    }
    if (new_pad&PSP_CTRL_RIGHT) {
    	if ((state_slot%2)<1) state_slot++;
    }
    if (new_pad&PSP_CTRL_UP) {
    	if (state_slot>1) state_slot-=2;
    }
    if (new_pad&PSP_CTRL_DOWN) {
    	if (state_slot<2) state_slot+=2;
    }
    if (new_pad&PSP_CTRL_CIRCLE) {
...
int menu_savestate(char *mode) {
	...
	for (;;) {		
		...
		mh_print_light(250,165,psp_msg_string(MENU_STATE_CHOOSESAVE),31|(31<<5)|(31<<10),menu_current_smoothing);
...
int menu_deletestate(char *mode) {
	...
	for (;;) {		
		...
		mh_print_light(250,165,psp_msg_string(MENU_STATE_CHOOSEDEL),31|(31<<5)|(31<<10),menu_current_smoothing);
		
		//now draw each slot
		if (state_slot<5) px=15;
		else px=15;
		py=50;
		for (i=0;i<4;i++) {					  	
			py=29+(i/2)*118;
...
int menu_credits(char *mode) {		
	...
	for (;;) {		
		...
		mh_printLimit(menu_panel_pos+5,15,479,272,"Code : Zack, YoyoFR, Laxer3a",CODE_COL);
		mh_printLimit(menu_panel_pos+5,25,479,272,"Official Forum : www.retroemu.com",CODE_COL);
		if (menu_panel_pos+5+40<479-40) pgDrawFrame(menu_panel_pos+5+40,35,479-40,35,CODE2_COL);
		if (menu_panel_pos+5+40<479-40) pgDrawFrame(menu_panel_pos+5+40,36,479-40,36,CODE3_COL);
		
		mh_printLimit(menu_panel_pos+5,40,479,272,"All Graphics by : Arcille",GFX_COL);
		mh_printLimit(menu_panel_pos+5,50,479,272,"Splines based on PSPSDK samples",GFX_COL);
		//mh_printLimit(menu_panel_pos+5,60,479,272,"Other bgs from internet",GFX_COL); //Room for two more credits here
		...
		mh_printLimit(menu_panel_pos+5,113,479,272,"Beta Testers : ",GREETINGS0_COL);
		mh_printLimit(menu_panel_pos+5,125,479,272,"Wally",GREETINGS_COL);
		mh_printLimit(menu_panel_pos+5,135,479,272,"Cmbeke",GREETINGS_COL);
		mh_printLimit(menu_panel_pos+5,145,479,272,"If you would like to apply",GREETINGS_COL);		
		mh_printLimit(menu_panel_pos+5,155,479,272,"to be a beta tester, logon to",GREETINGS_COL);
		mh_printLimit(menu_panel_pos+5,165,479,272,"www.retroemu.com",GREETINGS_COL);
		
		mh_printLimit(menu_panel_pos+5,185,479,272,"Greetings to (no order) : ",GREETINGS0_COL);
		mh_printLimit(menu_panel_pos+5,195,479,272,"snes9x authors",GREETINGS_COL);
		mh_printLimit(menu_panel_pos+5,205,479,272,"bifuteki (first snes9x port on PSP)",GREETINGS_COL);
		mh_printLimit(menu_panel_pos+5,215,479,272,"zelurker, tinnus, thunderz, smith",GREETINGS_COL);		
		mh_printLimit(menu_panel_pos+5,225,479,272,"Y, radius, Chris Swindle, Danzel",GREETINGS_COL);
		mh_printLimit(menu_panel_pos+5,235,479,272,"Ps2Dev, Supporters",GREETINGS_COL);
...
int menu_versioninfos(char *mode) {		
	...
	for (;;) {		
		...
		mh_printLimit(menu_panel_pos+5,104,479,272,EMUNAME_VERSION,CODE_COL);
#endif		
		sprintf(str_tmp,"Built on %s",__TIMESTAMP__);
  ...
  for (i=0;i<1;i++) {
	//for (i=-1;i<=MENU_XMB_ICONS_NB;i++) {
		...
		if ((sel>=0)&&(sel<MENU_XMB_ICONS_NB)) {
			...
			if ((x<480-64)&&(x>12)) {				
				if (i==0) {
					//if (selected) x-=64; //Dont scroll the icons over to the left (when going into sub menu) since its not needed anymore
	...
	if (j>=2) {//top is hidden		
		...
		//THE FLASHING UP ARROW
		mh_print(x+100,95,SJIS_UP " ",col);
	}
			...
			if (menu_xmb_entries[i].menu_id==menu_current_xmb_index_entry[menu_current_xmb_index]) {
				...
				if (menu_xmb_entries[i].menu_func) { //menu func available ?
					...
					if (str_tmp[0]) {//a value has to be shown
							//comment out print light if you want the underscore bar
						mh_print_light(15,100+y+4,psp_msg_string(menu_xmb_entries[i].label_id),31|(31<<5)|(31<<10),menu_current_smoothing);
						mh_print(15+120,100+y+4,str_tmp,31|(31<<5)|(31<<10));
						//comment out the two drawFrames if you want the underscore bar
						...
						mh_print_light(15,100+y+4,psp_msg_string(menu_xmb_entries[i].label_id),31|(31<<5)|(24<<10),menu_current_smoothing);
					}
				}
				//check for help to display
				if (menu_xmb_entries[i].help_id) { //help label available
				//HELP INFO
					msgBoxLinesRawPosLimit(245,165,200,67,psp_msg_string(menu_xmb_entries[i].help_id));
				}
				...
				if (menu_xmb_entries[i].menu_func) { //menu func available ?
					...
					menu_xmb_entries[i].menu_func(str_tmp);
					if (str_tmp[0]) {//a value has to be shown
						mh_print(15,100+y,psp_msg_string(menu_xmb_entries[i].label_id),20|(20<<5)|(20<<10));
						mh_print(15+120,100+y,str_tmp,20|(20<<5)|(20<<10));
					} else {
						mh_print(15,100+y,psp_msg_string(menu_xmb_entries[i].label_id),20|(20<<5)|(12<<10));
					}
				}
...
int root_menu(void) {
	...
	for (;;) {		
        ...
        if( readHomeButton() > 0 )
        {
	        if (psp_msg(ASK_EXIT,MSG_DEFAULT)) ExitCallback(0,0,NULL);;
        }
#endif				
				//process input									
				if (new_pad & PSP_CTRL_SELECT) {
