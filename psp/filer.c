...
#define MAXPATH 512		//Seems right
...
int getFilePath(char *out,int can_exit) {
  ...
  int counter=0;
	...
	int rows=21, top=0, x, y, h, i, /*len,*/ bMsg=0, up=0,pad_cnt=0,nopress=0,pad_cnt_acc=0;
	int pad;
	int retval;
	u16 *snes_image;
	int snesheight;
...
//			scePowerSetClockFrequency(222,222,111);
    ...
    if ((image_loaded==2)&&( nopress )) { //try new one if not keeping key pressed
        ...
        if (can_exit) mh_print(4,262,psp_msg_string(FILER_STATUS_CANEXIT1),INFOBAR_COL2);
        else mh_print(4,262,psp_msg_string(FILER_STATUS_NOEXIT1),INFOBAR_COL2);
        	
		if(nfiles > rows){
			h = 219;
			//pgDrawFrame(461,25,462,243,(0xd<<0)|(0xd<<5)|(0x17<<10));
			////////////////////////REPLACE ONE OF THESE WITH THE COIN BLOCK IMAGE AND COMMENT OUT THE OTHER (DOESNT MATTER WHICH)
			
			//image_put(465,h*top/nfiles+25,blockImg,255,0);
		...
		////HERE FOR ADJUSTING ROM FILENAMES DISPLAY
		x=15; y=34;
		for(i=0; i<rows; i++){
...
//    int counter=0;
	...
	unsigned long color=RGB_WHITE;
	static int sel=0;
	int rows=21, top=0, x, y, h, i, /*len,*/ bMsg=0, up=0,pad_cnt=5,pad_cnt_acc=0;
	int pad;
	int retval;
