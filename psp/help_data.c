...
static char help_data_ja[HELP_TOTAL_ENTRIES][1024]  __attribute__((aligned(16))) = {
	//japanese TODO : translation
		...
		"現在のゲーム画面をJPEG画像に保存\nブラウザーにアイコンとして利用されます。",
		"SFCリセット\n現在のゲーム情報が消えてしまうのでご注意。",
		"三つのモードが選べる、\n-エミュレートしない : 音なし、音CPUなし, ゲームによって動かせない。\n-エミュレートするが出力しない : 音なし、音CPU有り\n-エミュレートし、出力する : 音有り、音CPU有り",
		"音の品質を選ぶ、\n高ければ高いほど、音が良いが、エミュが遅くなります。",
		"描画エンジンの選択\n-ほぼ正確なソ\フトウェア描画\n-正確なソ\フトウェア描画\nPSPアクセラレータ\nPSPアクセラレータ + 正確なソ\フトウェア描画 : 複合モード\nPSPアクセラレータ + ほぼ正確なソ\フトウェア描画 : 複合モード\nPSPのハードウエアがすべてのスーファミの描画機能\を再現出来ないため、\nソ\フトウエアも使う複合モードが存在するのです。\n自動的に描画目的によって\nベストな描画ルーチンを選択するですが、\nほぼ正確なソ\フトウェア描画が速度を優先、正確なソ\フトウェア描画が本物と同じ描画を目指します。",
		"Snesの速度を目指すため、ある時に描画しないという選択も出来ます\n0の場合、すべてのフレームが描画\n1は１：１描画する（する、しないの切り替え）\n2は描画を１回する、２回しない...",
		"いくつ画面を描画するのに必要とされる描画関数を呼び出しているかを表\示します。\n低ければ、早くなります。これはハック(詳細は以下)の影響を確認するのに使えます。\n",
		"データベースに存在する場合SNESAdvanceスピードハックを使用します。\nROM読み込み時のみに使われます。",
